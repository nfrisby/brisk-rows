...
-- | Alias of 'prj#'
prjP :: (KnownLT nm rho, Found a ~ Find nm rho) => proxy nm -> Rcd f rho -> Sem f nm a
prjP = \nm -> prj# (name# nm)

-- prjP :: KnownLT nm (rho :& nm := a) => proxy nm -> Rcd f (rho :& nm := a) -> Sem f nm a
...
-- | Alias of 'inj#'
injP :: (KnownLT nm rho, Found a ~ Find nm rho) => proxy nm -> Sem f nm a -> Vrt f rho
injP = \nm -> inj# (name# nm)

-- injP :: KnownLT nm (rho :& nm := a) => proxy nm -> Sem f nm a -> Vrt f (rho :& nm := a)
